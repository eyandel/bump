...
class BumpHunter1D:
    ...
    def plot_tomography(self, bkg, is_hist: bool = False, label: str = '' , filename=None, chan: int = 0):
        ...
        if (isinstance(self.res_ar, np.ndarray) and self.res_ar.size == 0) or (not isinstance(self.res_ar, np.ndarray) and len(self.res_ar) == 0):
    ...
    def plot_bump(
        ...
        if multi_chan:
            ...
            if not is_hist:
                ...
                H = [data, np.histogram_bin_edges(data, bins=self.bins, range=self.rang)]
                #H = [data, self.bins]
    ...
    def bump_info(self, data, is_hist: bool=False):
        ...
        if isinstance(self.res_ar, np.ndarray) and self.res_ar.ndim == 2:
            multi_chan = True
        else:
            multi_chan = False
